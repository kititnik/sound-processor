...
void WavReader::validateRiffHeader(const RiffHeader& header) {
    if(memcmp(&header.chunkId, "RIFF", 4) != 0) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "RIFF header chunk Id", "RIFF", std::string_view(header.chunkId, 4)));
    }
    if(memcmp(&header.waveId, "WAVE", 4) != 0) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "RIFF header wave Id", "WAVE", std::string_view(header.waveId, 4)));
    }
}
...
void WavReader::validateFmtHeader(const FmtHeader& header) {
    if(memcmp(&header.chunkId, "fmt ", 4) != 0) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header chunk Id", "fmt ", std::string_view(header.chunkId, 4)));
    }
    if(header.wFormatTag != AUDIO_FORMAT) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header compression code", AUDIO_FORMAT, header.wFormatTag));
    }
    if(header.wChannels != CHANNELS_COUNT) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header number of channels", CHANNELS_COUNT, header.wChannels));
    }
    if(header.dwSamplesPerSec != SAMPLE_RATE) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header sample rate", SAMPLE_RATE, header.dwSamplesPerSec));
    }
    if(header.dwAvgBytesPerSec != BYTES_PER_SEC) {
        throw std::runtime_error(std::format(FILE_FORMAT_ERROR, "FMT header average bytes per second", BYTES_PER_SEC,
                                             header.dwAvgBytesPerSec));
    }
    if(header.wBlockAlign != BLOCK_ALIGN) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header block align", BLOCK_ALIGN, header.wBlockAlign));
    }
    if(header.wBitsPerSample != BITS_PER_SAMPLE) {
        throw std::runtime_error(
            std::format(FILE_FORMAT_ERROR, "FMT header bits per sample", BITS_PER_SAMPLE, header.wBitsPerSample));
    }
}
